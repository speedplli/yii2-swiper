...
Настройка параллакса произодится через поле `\lantongxue\yii2\swiper\Swiper::$parallaxOptions`, аналогично `\yii\helpers\BaseHtml::tag`

> Обратите внимание: чтобы параллакс заработал, 
  необходимо указать `parallax = true` в поле `\lantongxue\yii2\swiper\Swiper::$pluginOptions`
